...
# make a correction based on a direct message (21.01.25)
demog_all <- demog_all %>% 
  mutate(Age = ifelse(`Participant id`=="5d3a286034acef001ceb73ac", "26", Age))

# only those that we approved:
...
omega.search <- df_sbsa3 %>%
  ...
  psych::omega(m = ., key = ifelse(str_detect(keys_meaning_search[[1]], "-"), -1, 1), # function now requires the 1/-1 coding
               plot = FALSE, nfactors=3)
df_sbsa3$search <- df_sbsa3 %>%
  select(c("ml01_02", "ml01_03", "ml01_07", "ml01_08", "ml01_10")) %>%
  psych::reverse.code(keys = keys_meaning_search[[1]], items = .) %>%
  rowMeans(na.rm=T)
df_sbsa3$search[is.nan(df_sbsa3$search)] <- NA
